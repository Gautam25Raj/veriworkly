...
private AI policy loaded from configuration at boot (`AI_PRIVATE_CONFIG_PATH` or
`AI_PRIVATE_CONFIG_JSON`), not in application code. `GET /api/v1/ai/actions` exposes only the public
...
See the [AI API reference](/api-reference/ai) for the `/actions` (available actions and their costs)
