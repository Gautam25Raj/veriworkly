...
**Verdict:** the _sharing_ is good — genuinely better than most codebases of this shape. The _scaling_ is not. Nothing crashes at 50 documents, but the document dashboard freezes for ~half a second on every save, and four routes ship ~2.2 MB of PDF/DOCX engine they never call. Both have narrow, high-leverage fixes. See [Fix order](#fix-order).
...
- **One storage writer per type.** `getWorkspaceStorage()` (`document-workspace-service.ts:56`) memoizes a single `LocalStorageService` per document type, shared by autosave _and_ the sync worker. The two-independent-writers problem in the older `AUDIT.md` is fixed.
...
### 2.1 🔴 Autosave re-reads, re-validates and re-writes _every_ document on every keystroke — O(N)
...
`loadDocumentById()` (`document-workspace-service.ts:126`) calls `loadCollection()` and indexes into it — so it re-parses and re-zod-validates the _entire_ collection to fetch one item. With N documents that is N+1 full collection loads, i.e. N² document validations. Measured: **30 ms at 10, 150 ms at 25, 523 ms at 50.** Classic quadratic.
...
for (let count = 1; itemIndex + count <= items.length; count += 1) {
  ...
  if (fitsPage([...current, fragment])) acceptedCount = count;
  else break;
}
```
...
**Fix:** port `ResumePagedPreview` onto `paginateMeasuredItems` (lift it out of `cover-letter/shared.ts` into a shared paginator), and debounce the measurement effect ~150 ms. This is the single biggest typing-latency win in the editor and it _reduces_ code.
...
- The resume store is a global singleton holding _one_ document, so `resume.id` is ambient rather than a parameter. It works because only one editor mounts at a time, but nothing enforces that, and it's why `loadResume()` has to re-read the active-id key from localStorage (`resume-core.ts:44-56`).
...
The editor route still shows the export chunks in its module graph (2659 KB by that metric) — correctly, since it is the one place that _does_ export. The difference is that every path to `@react-pdf/renderer` and `docx` is now an `await import()`, so the browser fetches them when the user clicks Download rather than on page load. That is verifiable from the source: no module reachable without a dynamic import references either package.
...
## Round 2: resume / cover-letter alignment

The first round fixed scaling but left the two document types structurally different, and skipped the lazy template registries. Both are now done. Verification: **127 studio tests** (5 new), **parity suite 88 passed / 8 skipped — identical to the pre-change baseline**, typecheck + ESLint + Prettier clean, `next build` succeeds.

### Duplication removed

| Was                                                                                                                                                      | Now                                                                                                                               |
| -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `Field`/`TextField` defined **twice** with the same names and incompatible contracts (children-based vs value/onChange)                                  | one `features/documents/editor/form/` — both old modules deleted, 17 consumers migrated                                           |
| `SectionAccordion` in **three** places (resume content, cover letter panel reaching into the resume folder, a private copy inside `EditorSettingsPanel`) | one `features/documents/editor/SectionAccordion.tsx`, now with `aria-expanded` / `aria-controls` / `role="region"`                |
| Page height **1122** (resume) vs **1123** (cover letter) for the same A4 page                                                                            | one `templates/shared/page-geometry.ts`; the geometry contract test now asserts against the constant, not a literal               |
| Two paginators — resume O(items²), cover letter O(items)                                                                                                 | both on `paginateIncremental`; the now-dead `paginateMeasuredItems` deleted                                                       |
| `fitsInsideBottomPadding` copied into both cover letter templates                                                                                        | one `templates/cover-letter/measure.ts`                                                                                           |
| Resume had a template registry; cover letters had `if (templateId === …)` branches duplicated across `web.tsx`, `pdf.tsx`, and the HTML builder          | one `createTemplateRegistry` in `templates/shared/`, with `templates/resume/registry.ts` and `templates/cover-letter/registry.ts` |
| Save-failure wording inline in `ResumeToolbar`; cover letter had none                                                                                    | one `save-failure-message.ts`, used by both                                                                                       |
| Resume `ToolbarSecondaryActions`; cover letter a hand-rolled inline Save button                                                                          | one `ToolbarSaveButton`                                                                                                           |

### Behaviour the cover letter was missing

- **Quota failures were silent.** `useCoverLetterDocument` discarded every `saveDocument` result, so a full-storage failure lost edits with no message. The editor now surfaces it exactly as the resume does.
- **Delete used a bare `window.confirm()`** and never removed the cloud copy. It now uses the same `DestructiveModal` + `DocumentApi.delete` path as the resume, via a new `CoverLetterEditorModals` mirroring `ResumeEditorModals`.
- **No editable title.** The toolbar showed a static "Cover Letter Editor"; it now has the same inline-editable title as the resume, and Full Preview / PDF Debug moved into the shared actions menu instead of sitting as loose buttons.
- **No `useDeferredValue`**, so every keystroke re-ran the template's measuring pass at blocking priority.
- **State was local `useState`.** It is now `features/cover-letter/store/cover-letter-store.ts`, deliberately the same shape as the resume store, and the content/settings panels read narrow selectors instead of taking eight threaded props.

### Fixed for both

- **"Reset to Defaults" and "Empty Fields" had no confirmation** in either editor, despite wiping the whole document with no undo — while delete right below them did confirm. Both now confirm through a new lightweight `ConfirmModal` (distinct from `DestructiveModal`, which demands typing a verification word and warns about server-side purging — wrong register for a local reset).
- **Placeholder leak into real output.** An entry with a company and bullets but no role printed the literal word **"Role"** into the preview _and_ the exported PDF/DOCX/HTML; likewise "Project" and "Education". The render model now emits an empty title and the templates render a flex spacer instead of an empty heading, so the meta column stays aligned and preview and export agree.

### Item 7, reversed

I originally skipped the lazy template registries, arguing the marginal saving wasn't worth the churn. Done properly it was worth it: **the editor route dropped 2659 → 2583 KB and pdf-debug 2134 → 2086 KB**, the editor now fetches one template skin instead of six, and — more valuable than the bytes — cover letters gained a real registry, so adding a template is one entry in one file instead of edits to three branch statements.

Two things this surfaced that were worth having:

- `ParityClient` gates the harness on `data-parity-ready`. With lazy templates that flag had to include template resolution, or the harness could latch it while the page was still empty and measure nothing.
- `PdfDebugClient` registered PDF fonts in an effect while building the PDF element during render, so a first render could happen against the fallback font. Fonts are now registered immediately before the element is created.

### Verified stale

Two items carried over from the earlier `AUDIT.md` were already fixed in the current code; I confirmed rather than "fixed" them, and locked both in with tests:

- `formatDateRange` no longer emits `"Start - End"` (the _item titles_ still leaked, which is the bug fixed above).
- Export link hrefs are safe by construction: `normalizeLinkHref` allowlists `http(s)`/`mailto`/`tel` and prefixes everything else with `https://`, rendering `javascript:`/`data:`/`vbscript:` inert — including whitespace-split variants, since `cleanResumeText` collapses whitespace.

### Still open

`Sidebar.tsx` and `AdvancedThemeSettings.tsx` remain dead code, and most `saveDocument` callers outside the two editors still discard the result. Neither is a resume/cover-letter asymmetry.

---

### One deliberate deviation (round 1)

**Item 7 (lazy template registries) was not done, on purpose.** Once item 2 landed, `templates/resume/pdf` became reachable only through `export-pdf`, which is already dynamically imported — so the PDF templates are behind a lazy boundary regardless. Splitting the six _web_ templates would add six chunk round trips to save ~85 lines of skin code each, while `templates/resume/shared/web.tsx` (the actual weight) is needed by whichever template is selected anyway. It would also have churned eight test files. The misleading comment that claimed these imports were already dynamic is now corrected to describe what the code actually does and why.
