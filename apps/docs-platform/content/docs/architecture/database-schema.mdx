...
<Callout type="info" title="Two different audit tables">
  `AdminAuditEntry` is the live admin action log. `AuditLog` is a separate, generic HTTP audit table
  that is modelled but currently has no writer anywhere in the server source — it is effectively
  unused today.
</Callout>
