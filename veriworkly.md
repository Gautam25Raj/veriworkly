...
Full section-based editor: Basics, Summary, Experience, Education, Projects, Skills, Links, Languages, Interests, Awards, Certifications, Publications, References, Volunteer, Achievements, plus custom/generic sections. Sections can be shown/hidden individually and reordered by **two working mechanisms**: drag-and-drop on the section cards in the left "Editor Map" sidebar (`Sidebar.tsx`, genuinely `draggable`), and a "Position" dropdown plus drag handles inside the section-visibility settings panel. _(An earlier version of this doc claimed the sidebar drag implementation was dead code that had never been made `draggable` — that is no longer true; it works.)_ A paged, live, pixel-accurate preview updates as you type. Two resume templates today: **Precision ATS** (id `precision-ats`, dense single-column) and **Executive Clarity** (id `executive-clarity`, refined spacing) — both freely selectable, no template lock for resumes.
...
An internal admin area (`/admin/*`, gated by a real role check — `requireAdminUser()` — not just a client-side hide) covering:

- **Usage metrics dashboard** — today's counts and all-time totals for events like resumes created/exported, logins, OTP sends, dashboard/roadmap views.
- **GitHub repo stats** — the VeriWorkly open-source repo's own issue/PR/completion stats, synced roughly every 12 hours, shown for transparency/roadmap purposes — unrelated to any individual user's GitHub import.
- **Monetization console** (`/admin/monetization`) — manual credit grants, manual entitlement grants, manual affiliate-commission creation, an affiliate-tier/status moderation panel, a withdrawal-request approval/rejection queue, and an audit log of admin actions.
- **Roadmap CMS** (`/admin/roadmap`) — create/edit/delete public roadmap items with rich detail: problem/solution narrative, timeline, impact metrics, before/after comparisons.
- **Changelog CMS** — admin create/update/delete plus a manual GitHub-Releases sync trigger, exposed at `POST|PUT|DELETE /changelog/admin*` and `POST /changelog/admin/sync`. No dedicated Studio screen yet; these are API-only.
- **Ambassador application review** — **the API is live** (`GET /admin/ambassador`, `PATCH /admin/ambassador/:id`), admin-gated, transactional, and audit-logged. There is still no dedicated Studio _screen_ for it, unlike the affiliate moderation panel. _(An earlier version of this doc said the whole review workflow was "the next admin-tooling milestone" and not yet built — the backend half has shipped.)_ See [Affiliate & Ambassador Programs](#affiliate--ambassador-programs).
...
**Admin review — live.** `GET /admin/ambassador` lists applications (optionally filtered by status, newest first, joined with the applicant's id/email/name/role); `PATCH /admin/ambassador/:id` takes `action: "APPROVE" | "REJECT"` plus an optional `reviewNote` (≤500 chars). Both sit behind `adminAuthMiddleware`. Approving flips the application status **and** the user's `role` to `AMBASSADOR` in one transaction; rejecting sets the role back to `USER`. Every decision writes an `AdminAuditEntry` (`ambassador.application.approve` / `.reject`). An already-reviewed application cannot be reviewed twice — a second `PATCH` returns `400`. There is no admin _UI screen_ for this yet.
...
- **Admin write routes:** `POST /changelog/admin`, `PUT /changelog/admin/:id`, `DELETE /changelog/admin/:id`, plus `POST /changelog/admin/sync` to pull missing GitHub Releases on demand.
...
10. **~~Ambassador program's rewards layer is the next rollout milestone~~ — PARTIALLY RESOLVED.** The **admin-review workflow has shipped** (`GET /admin/ambassador`, `PATCH /admin/ambassador/:id`, transactional, audit-logged), and the application schema grew from 2 fields to 7 behind a dedicated `AmbassadorApplication` model. What remains genuinely unbuilt: the points economy, `.edu` verification, voucher redemption, and the ambassador leaderboard — none of which have any database model. Also still missing: an admin _UI screen_ for reviews, and the ambassador rewards dashboard in Studio. See [Affiliate & Ambassador Programs](#affiliate--ambassador-programs).
