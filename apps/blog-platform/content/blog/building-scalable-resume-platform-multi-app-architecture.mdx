...
3. Portfolio Builder (`apps/portfolio`)
4. Docs Platform (`apps/docs-platform`)
5. Blog Platform (`apps/blog-platform`)
6. API Server (`apps/server`)
...
- Share links, sync endpoints, and the AI/ATS/billing services behind them
- Scheduled jobs for GitHub sync, usage-metrics flushing, and portfolio access management

### Portfolio Builder (`apps/portfolio`)

- The public template gallery, authenticated portfolio editor, and the published sites themselves
- Server-rendered templates served on `*.veriworkly.com` subdomains
...
If you want to explore implementation details next, start with the [monorepo architecture docs](https://docs.veriworkly.com/docs/architecture/monorepo).
