...
<Steps>
  <Step>
    ...
    <Callout type="warn" title="apps/portfolio requires a private submodule">
      ...
      `git submodule update --init --recursive`. If you do not, skip it — every workspace except
      `apps/portfolio` builds and runs normally without it.
    </Callout>
...
| `npm run generate:api`                         | Bundle `apps/docs-platform/specs/openapi.yaml` and regenerate the API reference MDX pages. |
